  ...
  implements CartDao
{
  ...
  async createCart(): Promise<Cart> {
    const newCart = await super.create({ products: [] })
    return newCart
  }
  ...
  async saveProductToCart(cartId: string, productId: string): Promise<void> {
    ...
    logger.info(`Product Saved in Cart: ${JSON.stringify(updatedCart)}`)
  }
  ...
  async updateCart(cartId: string, data: any): Promise<Cart> {
    ...
    data.forEach((product: any) => {
      const productIndex = cart.products.findIndex(
        (p: any) => p.id.toString() === product.id.toString()
      )
      if (productIndex !== -1) {
  ...
  async updateProductInCart(
    ...
    const { cart, productIndex } = await this.findProductInCart(
      cartId,
      productId
    )
  ...
  private async findProductInCart(
    cartId: string,
    productId: string
  ): Promise<any> {
    const cart = await super.findById(cartId)
  ...
  async purchase(cartId: string, userEmail: string): Promise<Cart | any> {
    ...
    try {
      ...
      if (cart.products.length === 0)
        throw new createError.BadRequest(`Cart without products`)
  ...
  private calculatePurchaseOrder(cart: Cart): any {
